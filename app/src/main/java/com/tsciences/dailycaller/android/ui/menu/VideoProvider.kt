...
class VideoProvider {
    ...
    companion object {
                        ...
                        for (j in 0 until videos.length()) {
                            ...
                            for (k in 0 until videoSpecs.length()) {
                                ...
                                if (TARGET_FORMAT == videoSpec.getString(TAG_VIDEO_TYPE)) {
                                    videoUrl = urlPrefixMap[TARGET_FORMAT] + videoSpec.getString(
                                        TAG_VIDEO_URL
                                    )
                            ...
                            val bigImageUrl =
                                urlPrefixMap[TAG_IMAGES] + video.getString(TAG_IMG_780_1200)
                                    ...
                                    for (k in 0 until tracksArray.length()) {
                                        ...
                                        tracks.add(
                                            buildTrack(
                                                ...
                                                urlPrefixMap[TAG_TRACKS] + track.getString(
                                                    TAG_TRACK_CONTENT_ID
                                                ),
        ...
        private fun buildMediaInfo(
            ...
            var mediaInfo: MediaInfo.Builder =
                MediaInfo.Builder(url).setStreamType(MediaInfo.STREAM_TYPE_BUFFERED)
                    .setContentType(mimeType!!).setMetadata(movieMetadata)
                    .setStreamDuration((duration * 1000).toLong()).setCustomData(jsonObj!!)
        ...
        private fun buildTrack(
            ...
            return MediaTrack.Builder(id, trackType).setName(name).setSubtype(trackSubType)
                .setContentId(contentId).setLanguage(language).build()
        }
    }
}